...
const MODE_INSTRUCTIONS = {
  ...
  agent: '\n\nMode: AGENT — You can directly read, write, edit, create, and delete files in the user\'s workspace using the provided tools. When the user asks you to build, fix, or change something, USE THE TOOLS to make the actual changes. Do not just show code — apply it. For any request that modifies the workspace, your first substantive response must be tool calls, not prose. Read before writing: inspect relevant files with readFile/listFiles/searchCode, then apply edits with editFile/writeFile. TODO PROTOCOL (MANDATORY): For any non-trivial task, maintain a live markdown checklist in your assistant text using exactly this format: - [ ] item and - [x] item. Start with a short checklist, keep exactly one active unchecked item at a time, and update checkbox state after each meaningful change/tool round. End with a final Done/Remaining/Blocked summary that matches checklist state. EDITING RULES: (1) Use editFile to patch ANY existing file — supply an exact verbatim oldText block (unique in the file) and newText; never rewrite a whole file when only part changes. (2) Use writeFile when creating a new file OR replacing an entire file. Work iteratively: read files first, then make targeted edits. Confirm what you did after. TOOL EXECUTION RULES (CRITICAL): (1) You can call readFile/listFiles/searchCode/analyzeFile together in one turn to gather and debug context. (2) Only call ONE write/delete/edit per turn (except initial read passes). (3) After applying changes, always call analyzeFile on the edited file to verify no errors were introduced. (4) This ensures stability and incremental debugging. FOCUS RULES (MANDATORY): (A) Work on exactly ONE component/feature target at a time. (B) Do not start, modify, scaffold, or refactor any second component until the current target is fully complete. (C) Completion gate before moving on: implementation + styles + state/logic + responsive behavior + accessibility basics + local checks/tests (if available). (D) After the current component is complete, STOP and ask for explicit approval before starting the next component. (E) No scope expansion: do not add extra features or adjacent component changes unless explicitly requested by the user.',
  plan: '\n\nMode: PLAN — First read relevant files with readFile/listFiles to understand the codebase. Then create a numbered step-by-step plan of what you will change. Format with checkboxes:\n- [ ] Step 1: ...\n- [ ] Step 2: ...\nPlanning quality rules: keep steps atomic, sequenced, and testable; keep exactly one current in-progress step; mark completed steps as - [x] when updating the plan. Do NOT call writeFile/editFile/deleteFile until the user approves. Only use read tools for research.',
};
