...
class Bridge {
  ...
  async boot(opts: BootOptions): Promise<WebContainer> {
    ...
    this.bootPromise = (async () => {
      ...
      try {
        // Do NOT specify workdirName. Passing workdirName: 'epicodespace' causes
        // server.build() inside the stackblitz.com headless iframe to throw
        // ENOENT on Safari/iPadOS because the server tries to create
        // /home/epicodespace before /home exists in the WASM VFS.
        // Let WebContainers auto-generate a safe workdir, then mount there.
        ...
        // ── Step 2: mount ─────────────────────────────────────────────────
        // Mount into container.workdir (auto-generated by WebContainers) so
        // files land exactly where the shell's default cwd is.
        const workdir = rawContainer.workdir;
        const tree = buildTreeFromFlat(opts.files);
        logger.info('runtime', 'calling mount()', { mountPoint: workdir, keys: Object.keys(tree) });
        try {
          await rawContainer.mount(tree, { mountPoint: workdir });
        } catch (mountErr) {
  ...
  async syncFiles(files: BootOptions['files']): Promise<void> {
    ...
    await c.mount(tree, { mountPoint: c.workdir });
