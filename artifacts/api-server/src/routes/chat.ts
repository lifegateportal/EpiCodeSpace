...
const WORKSPACE_TOOLS = [
  { name: 'readFile', description: 'Read file contents. Prefer startLine/endLine for focused chunks. Do not reread the active file if it is already in context. Before the first edit, keep reads to the primary file plus only the minimum supporting files needed to disambiguate the change.', parameters: { type: 'object', properties: { path: { type: 'string' }, startLine: { type: 'number', description: 'Optional 1-indexed start line (for chunk reads)' }, endLine: { type: 'number', description: 'Optional 1-indexed end line (for chunk reads)' }, maxChars: { type: 'number', description: 'Optional hard cap for returned characters (default 60000)' } }, required: ['path'] } },
  { name: 'writeFile', description: 'Create or fully overwrite a file. Use only for a targeted file change, not as a shortcut for broad rewrites or folder resets.', parameters: { type: 'object', properties: { path: { type: 'string' }, content: { type: 'string' } }, required: ['path', 'content'] } },
  ...
  { name: 'deleteFile', description: 'Delete a file from the workspace. Only use when the user explicitly requests deleting that specific file or when replacing a generated file is unquestionably required.', parameters: { type: 'object', properties: { path: { type: 'string' } }, required: ['path'] } },
